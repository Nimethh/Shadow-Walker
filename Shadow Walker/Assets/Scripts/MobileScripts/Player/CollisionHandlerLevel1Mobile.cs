...
public class CollisionHandlerLevel1Mobile : RaycastController
{
    [HideInInspector]
    public Vector2 playerInput = Vector2.zero;

    public Vector3 spawningPosition = Vector3.zero;
    List<Vector3> previousSpawningPosition = new List<Vector3>();
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        ...
        if (other.gameObject.CompareTag("CheckPoint"))
        {
            ...
            checkPointPosition.z = -3.0f;
            if (checkPointPosition != spawningPosition && !previousSpawningPosition.Contains(checkPointPosition))
            {
                audioManager.Play("SafePoint");
                Animator animator = other.gameObject.transform.GetChild(0).GetComponent<Animator>();
    ...
    private void OnTriggerStay2D(Collider2D other)
    {
        if (other.gameObject.CompareTag("CheckPoint"))
        {
            ...
            if (checkPointPosition != spawningPosition && !previousSpawningPosition.Contains(checkPointPosition))
            {
                previousSpawningPosition.Add(checkPointPosition);
                spawningPosition = checkPointPosition;
            }
